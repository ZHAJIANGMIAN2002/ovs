...
#### Using a Different Backbone (e.g., PTV3)
We have designed the training script to be flexible, allowing for the use of different 3D backbones. As an example, we have integrated the Point Transformer V3 (PTV3) backbone. To train a model using PTV3:

1.  **Ensure Pointcept is in your PYTHONPATH**: The PTV3 model is part of the `Pointcept` library, which should be included as a subdirectory in this project. Make sure the root of the `openscene` project directory is added to your `PYTHONPATH` so that the scripts can find the `Pointcept` modules.
    ```bash
    export PYTHONPATH=$(pwd):$PYTHONPATH
    ```

2.  **Use the PTV3 config file**: We have provided a specific configuration file to run training with PTV3.
    ```bash
    # To start training with PTV3 on ScanNet
    sh run/distill.sh out/scannet_ptv3 config/scannet/train_ptv3.yaml
    ```
    The main training script will automatically detect the architecture specified in `train_ptv3.yaml` and load the appropriate models. You can inspect this yaml file to see how to specify different architectures and their corresponding configurations.
